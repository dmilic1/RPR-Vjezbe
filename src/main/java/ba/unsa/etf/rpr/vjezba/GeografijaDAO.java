...
public class GeografijaDAO {
    ...
    private PreparedStatement glavniGradUpit, dajDrzavuUpit, obrisiDrzavuUpit, obrisiGradoveZaDrzavu, nadjiDrzavu,
    ...
    private GeografijaDAO(){
        ...
        try {
            glavniGradUpit = conn.prepareStatement("SELECT grad.id, grad.naziv, grad.broj_stanovnika, grad.drzava FROM grad, drzava WHERE drzava.id = grad.drzava AND drzava.glavni_grad = ?");
        } catch (SQLException e) {
            regenerisiBazu();
            try {
                glavniGradUpit = conn.prepareStatement("SELECT grad.id, grad.naziv grad.broj_stanovnika, grad.drzava FROM grad, drzava WHERE drzava.id = grad.drzava AND drzava.glavni_grad = ?");
        ...
        try {
            ...
            nadjiDrzavu = conn.prepareStatement("SELECT * FROM drzava WHERE naziv =?");
    ...
    private void regenerisiBazu() throws FileNotFoundException {
